...
- **标签页拖拽换位**：按住标签头拖动时出现跟随鼠标的半透明“影子”，并用竖直指示线标出落点，松开鼠标才落位（类似桌面拖图标）。
- **同名标签自动区分**：多个标签末尾文件夹同名时（如两处 `2026-09-15`），标题自动加父级括注（如 `2026-09-15 (慧医卡)`）；悬停标签可看完整路径。
- **吸收后保留选中项**：在别的软件里“打开文件所在位置”时，资源管理器会选中该文件；被吸收成标签页后仍会选中它。
- **吸收资源管理器窗口**：自动捕获系统里新打开的 explorer 窗口并转成标签页；新窗口一创建就先隐藏，吸收完成前不显示，避免“一闪而过”。
...
└── ExplorerWindowWatcher.cs       # 监视并“吸收”资源管理器窗口的核心类（创建即隐藏，消除闪烁）
```
